...
void PortMenu::AddEnhancements() {
    ...
    AddWidget(path, "Minimap Y: %.0f px", WIDGET_CVAR_SLIDER_FLOAT)
        ...
        .Options(FloatSliderOptions()
                     ...
                     .Step(1.0f)
                     .Format("%.0f")
                     .Tooltip("Sets the one-player minimap center Y while its right edge follows the HUD safe zone."));
    AddWidget(path, "Minimap right pad: %.0f px", WIDGET_CVAR_SLIDER_FLOAT)
        .CVar("gArcadeKart.Hud.MinimapRightPad")
        .Options(FloatSliderOptions()
                     .Min(0.0f)
                     .Max(80.0f)
                     .DefaultValue(48.0f)
                     .Step(1.0f)
                     .Format("%.0f")
                     .Tooltip("Additional padding between the one-player minimap right edge and screen right safe zone."));
