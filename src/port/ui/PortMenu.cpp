...
void PortMenu::AddEnhancements() {
    ...
    AddWidget(path, "RPM red zone", WIDGET_CVAR_CHECKBOX)
        .CVar("gArcadeKart.Hud.RpmRedZoneEnabled")
        .Options(CheckboxOptions().DefaultValue(true).Tooltip("Draws the RPM warning zone as a runtime angular mask."));
    AddWidget(path, "RPM zero angle: %.0f deg", WIDGET_CVAR_SLIDER_FLOAT)
        .CVar("gArcadeKart.Hud.RpmMeterZeroDegrees")
        .Options(FloatSliderOptions()
                     .Min(0.0f)
                     .Max(360.0f)
                     .DefaultValue(230.0f)
                     .Step(1.0f)
                     .Format("%.0f")
                     .Tooltip("Angle used by zero RPM when mapping character shift stats onto the meter."));
    AddWidget(path, "RPM max angle: %.0f deg", WIDGET_CVAR_SLIDER_FLOAT)
        .CVar("gArcadeKart.Hud.RpmMeterMaxDegrees")
        .Options(FloatSliderOptions()
                     .Min(0.0f)
                     .Max(360.0f)
                     .DefaultValue(100.0f)
                     .Step(1.0f)
                     .Format("%.0f")
                     .Tooltip("Angle used by the top of the RPM meter when mapping character shift stats."));
    AddWidget(path, "RPM red zone bleed: %.2f deg", WIDGET_CVAR_SLIDER_FLOAT)
        .CVar("gArcadeKart.Hud.RpmRedZoneEdgeBleedDegrees")
        .Options(FloatSliderOptions()
                     .Min(0.0f)
                     .Max(4.0f)
                     .DefaultValue(0.75f)
                     .Step(0.25f)
                     .Format("%.2f")
                     .Tooltip("Small red-zone overdraw that covers filtered edge pixels cleanly."));
    AddWidget(path, "RPM red zone alpha: %.0f", WIDGET_CVAR_SLIDER_FLOAT)
        .CVar("gArcadeKart.Hud.RpmRedZoneAlpha")
        .Options(FloatSliderOptions()
                     .Min(0.0f)
                     .Max(255.0f)
                     .DefaultValue(255.0f)
                     .Step(5.0f)
                     .Format("%.0f")
                     .Tooltip("Opacity for the runtime RPM warning mask."));
