...
void PortMenu::AddEnhancements() {
    ...
    AddWidget(path, "Debug quick HUD race boot", WIDGET_CVAR_CHECKBOX)
        ...
        .Options(CheckboxOptions()
                     .DefaultValue(true)
                     .Tooltip("Boots directly into 1P 50cc Grand Prix on Yoshi Valley as Mario for HUD iteration."));
    ...
    AddWidget(path, "Logitech SDK spring: %.0f%%", WIDGET_CVAR_SLIDER_FLOAT)
        .CVar("gArcadeKart.LogitechSdkSpringPercent")
        .Options(FloatSliderOptions()
                     .Min(0.0f)
                     .Max(100.0f)
                     .DefaultValue(96.0f)
    ...
    AddWidget(path, "Use Logitech SDK dynamic spring force", WIDGET_CVAR_CHECKBOX)
        ...
        .Options(CheckboxOptions()
                     .DefaultValue(false)
                     .Tooltip("Adds the SDK spring force effect on top of preferred spring settings."));
    ...
    AddWidget(path, "RPM meter motion max: %.0f RPM", WIDGET_CVAR_SLIDER_FLOAT)
        .CVar("gArcadeKart.Hud.RpmMeterMotionMaxRpm")
        .Options(FloatSliderOptions()
                     .Min(4000.0f)
                     .Max(10000.0f)
                     .DefaultValue(7200.0f)
                     .Step(100.0f)
                     .Format("%.0f")
                     .Tooltip("Actual RPM that maps to full needle sweep. The meter still starts from zero."));
    AddWidget(path, "RPM character range scale: %.2f", WIDGET_CVAR_SLIDER_FLOAT)
        .CVar("gArcadeKart.Hud.RpmMeterCharacterRangeScale")
        .Options(FloatSliderOptions()
                     .Min(0.0f)
                     .Max(1.0f)
                     .DefaultValue(0.35f)
                     .Step(0.05f)
                     .Format("%.2f")
                     .Tooltip("Uses character torque stats to nudge each kart's RPM meter motion range."));
    AddWidget(path, "Reverse drive: %.2fx", WIDGET_CVAR_SLIDER_FLOAT)
        .CVar("gArcadeKart.ReverseDriveMultiplier")
        .Options(FloatSliderOptions()
                     .Min(0.10f)
                     .Max(1.50f)
                     .DefaultValue(0.75f)
                     .Step(0.05f)
                     .Format("%.2f")
                     .Tooltip("Scales throttle torque while manual reverse gear is selected."));
    AddWidget(path, "Reverse speed ratio: %.2f", WIDGET_CVAR_SLIDER_FLOAT)
        .CVar("gArcadeKart.ReverseSpeedRatio")
        .Options(FloatSliderOptions()
                     .Min(0.05f)
                     .Max(0.70f)
                     .DefaultValue(0.44f)
                     .Step(0.01f)
                     .Format("%.2f")
                     .Tooltip("Reverse gear speed cap as a fraction of the kart's forward top speed."));
    AddWidget(path, "RPM needle input scale: %.2fx", WIDGET_CVAR_SLIDER_FLOAT)
        .CVar("gArcadeKart.Hud.RpmNeedleInputScale")
        .Options(FloatSliderOptions()
                     .Min(0.25f)
                     .Max(12.0f)
                     .DefaultValue(1.0f)
                     .Step(0.25f)
                     .Format("%.2f")
                     .Tooltip("Scales actual engine RPM before mapping it into the needle sweep."));
    AddWidget(path, "RPM needle sweep: %.0f deg", WIDGET_CVAR_SLIDER_FLOAT)
        .CVar("gArcadeKart.Hud.RpmNeedleSweepDegrees")
        .Options(FloatSliderOptions()
                     .Min(10.0f)
                     .Max(180.0f)
                     .DefaultValue(100.0f)
                     .Step(1.0f)
                     .Format("%.0f")
                     .Tooltip("Clockwise degrees from the needle's zero position to full scale."));
    AddWidget(path, "Placement HUD scale: %.2f", WIDGET_CVAR_SLIDER_FLOAT)
        .CVar("gArcadeKart.Hud.PlaceNumberScale")
        .Options(FloatSliderOptions()
                     .Min(0.25f)
                     .Max(2.0f)
                     .DefaultValue(1.0f)
                     .Step(0.05f)
                     .Format("%.2f")
                     .Tooltip("Scales the placement-number HUD cluster."));
    AddWidget(path, "Placement HUD left offset: %.0f px", WIDGET_CVAR_SLIDER_FLOAT)
        .CVar("gArcadeKart.Hud.PlaceNumberLeftCorrection")
        .Options(FloatSliderOptions()
                     .Min(0.0f)
                     .Max(40.0f)
                     .DefaultValue(20.0f)
    ...
    AddWidget(path, "Placement HUD top offset: %.0f px", WIDGET_CVAR_SLIDER_FLOAT)
        .CVar("gArcadeKart.Hud.PlaceNumberTopCorrection")
        .Options(FloatSliderOptions()
                     .Min(0.0f)
                     .Max(40.0f)
                     .DefaultValue(10.0f)
                     ...
                     .Format("%.0f")
                     .Tooltip("Adjusts how far the placement number sits from the left/top anchors."));
    AddWidget(path, "Portrait strip scale: %.2f", WIDGET_CVAR_SLIDER_FLOAT)
        .CVar("gArcadeKart.Hud.PortraitStripScale")
        .Options(FloatSliderOptions()
                     .Min(0.25f)
                     .Max(2.0f)
                     .DefaultValue(0.8f)
                     .Step(0.05f)
                     .Format("%.2f")
                     .Tooltip("Scales the top-four portrait strip HUD cluster."));
    AddWidget(path, "Portrait strip side pad: %.0f px", WIDGET_CVAR_SLIDER_FLOAT)
        .CVar("gArcadeKart.Hud.PortraitStripSidePad")
        .Options(FloatSliderOptions()
                    .Min(0.0f)
                    .Max(80.0f)
                    .DefaultValue(70.0f)
                    .Step(1.0f)
                    .Format("%.0f")
                    .Tooltip("Offsets the portrait strip away from the left edge of the HUD."));
    AddWidget(path, "Portrait strip spacing: %.0f px", WIDGET_CVAR_SLIDER_FLOAT)
        .CVar("gArcadeKart.Hud.PortraitStripSpacing")
        .Options(FloatSliderOptions()
                     .Min(0.0f)
                     .Max(24.0f)
                    .DefaultValue(10.0f)
                    .Step(1.0f)
                    .Format("%.0f")
                    .Tooltip("Spacing between portraits in the top-four strip."));
    AddWidget(path, "Portrait strip Y offset: %.0f px", WIDGET_CVAR_SLIDER_FLOAT)
        .CVar("gArcadeKart.Hud.PortraitStripCenterYOffset")
        .Options(FloatSliderOptions()
                    .Min(-32.0f)
                    .Max(32.0f)
                    .DefaultValue(0.0f)
                    .Step(1.0f)
                    .Format("%.0f")
                    .Tooltip("Fine-tunes the portrait strip up or down from center-left anchor."));
    AddWidget(path, "Time/lap scale: %.2f", WIDGET_CVAR_SLIDER_FLOAT)
        .CVar("gArcadeKart.Hud.TimeLapScale")
        .Options(FloatSliderOptions()
                     .Min(0.25f)
                     .Max(2.0f)
                     .DefaultValue(0.8f)
                     .Step(0.05f)
                     .Format("%.2f")
                     .Tooltip("Scales the top-center race time and lap HUD cluster."));
    AddWidget(path, "Lap time list scale: %.2f", WIDGET_CVAR_SLIDER_FLOAT)
        .CVar("gArcadeKart.Hud.LapTimesScale")
        .Options(FloatSliderOptions()
                     .Min(0.25f)
                     .Max(2.0f)
                     .DefaultValue(0.8f)
                     .Step(0.05f)
                     .Format("%.2f")
                     .Tooltip("Scales the top-right lap time list HUD cluster."));
    AddWidget(path, "Minimap scale: %.2f", WIDGET_CVAR_SLIDER_FLOAT)
        .CVar("gArcadeKart.Hud.MinimapScale")
        .Options(FloatSliderOptions()
                     .Min(0.25f)
                     .Max(2.0f)
                     .DefaultValue(0.8f)
                     .Step(0.05f)
                     .Format("%.2f")
                     .Tooltip("Scales the minimap and its dots as one HUD cluster."));
    AddWidget(path, "RPM meter scale: %.2f", WIDGET_CVAR_SLIDER_FLOAT)
        .CVar("gArcadeKart.Hud.RpmMeterScale")
        .Options(FloatSliderOptions()
                     .Min(0.25f)
                     .Max(2.0f)
                     .DefaultValue(0.8f)
                     .Step(0.05f)
                     .Format("%.2f")
                     .Tooltip("Scales the RPM meter HUD cluster."));
    AddWidget(path, "HUD safe-zone X margin: %.0f px", WIDGET_CVAR_SLIDER_FLOAT)
        .CVar("gArcadeKart.Hud.SafeZoneX")
        .Options(FloatSliderOptions()
                     .Min(0.0f)
                     .Max(64.0f)
                     .DefaultValue(28.0f)
                     .Step(1.0f)
                     .Format("%.0f")
                     .Tooltip("Sets the left/right anchor margin used by the ArcadeKart HUD safe zone."));
    AddWidget(path, "HUD safe-zone top margin: %.0f px", WIDGET_CVAR_SLIDER_FLOAT)
        .CVar("gArcadeKart.Hud.SafeZoneTop")
        .Options(FloatSliderOptions()
                     .Min(0.0f)
                     .Max(32.0f)
                     .DefaultValue(0.0f)
                     .Step(1.0f)
                     .Format("%.0f")
                     .Tooltip("Sets the top anchor margin used by the ArcadeKart HUD safe zone."));
    AddWidget(path, "HUD safe-zone bottom margin: %.0f px", WIDGET_CVAR_SLIDER_FLOAT)
        .CVar("gArcadeKart.Hud.SafeZoneBottom")
        .Options(FloatSliderOptions()
                     .Min(0.0f)
                     .Max(48.0f)
                     .DefaultValue(10.0f)
                     .Step(1.0f)
                     .Format("%.0f")
                     .Tooltip("Sets the bottom anchor margin used by the ArcadeKart HUD safe zone."));
    AddWidget(path, "Minimap X: %.0f px", WIDGET_CVAR_SLIDER_FLOAT)
        .CVar("gArcadeKart.Hud.MinimapOnePlayerX")
        .Options(FloatSliderOptions()
                     .Min(220.0f)
                     .Max(310.0f)
                     .DefaultValue(238.0f)
                     .Step(1.0f)
                     .Format("%.0f")
                     .Tooltip("Sets the one-player minimap center X."));
    AddWidget(path, "Minimap Y: %.0f px", WIDGET_CVAR_SLIDER_FLOAT)
        .CVar("gArcadeKart.Hud.MinimapY")
        .Options(FloatSliderOptions()
                     .Min(48.0f)
                     .Max(180.0f)
                     .DefaultValue(112.0f)
                     .Step(1.0f)
                     .Format("%.0f")
                     .Tooltip("Sets the one-player minimap center Y."));
    AddWidget(path, "Top HUD X offset: %.0f px", WIDGET_CVAR_SLIDER_FLOAT)
        .CVar("gArcadeKart.Hud.TopXOffset")
        .Options(FloatSliderOptions()
                     .Min(-80.0f)
                     .Max(80.0f)
                     .DefaultValue(0.0f)
                     .Step(1.0f)
                     .Format("%.0f")
                     .Tooltip("Horizontal adjustment for the top time/lap cluster."));
    AddWidget(path, "Lap times right pad: %.0f px", WIDGET_CVAR_SLIDER_FLOAT)
        .CVar("gArcadeKart.Hud.LapTimesRightPad")
        .Options(FloatSliderOptions()
                     .Min(0.0f)
                     .Max(80.0f)
                     .DefaultValue(4.0f)
                     .Step(1.0f)
                     .Format("%.0f")
                     .Tooltip("Offsets the vertical lap-time list away from the right edge."));
    AddWidget(path, "Lap times top pad: %.0f px", WIDGET_CVAR_SLIDER_FLOAT)
        .CVar("gArcadeKart.Hud.LapTimesTopPad")
        .Options(FloatSliderOptions()
                     .Min(0.0f)
                     .Max(80.0f)
                     .DefaultValue(18.0f)
                     .Step(1.0f)
                     .Format("%.0f")
                     .Tooltip("Offsets the vertical lap-time list down from the top edge."));
    AddWidget(path, "Top HUD Y offset: %.0f px", WIDGET_CVAR_SLIDER_FLOAT)
        .CVar("gArcadeKart.Hud.TopY")
        .Options(FloatSliderOptions()
                     .Min(0.0f)
                     .Max(40.0f)
                     .DefaultValue(17.0f)
                     .Step(1.0f)
                     .Format("%.0f")
                     .Tooltip("Vertical adjustment for the top time/lap cluster."));
    ...
    AddWidget(path, "Post FX under HUD", WIDGET_CVAR_CHECKBOX)
        .CVar("gArcadeKart.PostFx.LayerHud")
        .Options(CheckboxOptions()
                     .DefaultValue(true)
    ...
    AddWidget(path, "Post FX shake response power: %.2f", WIDGET_CVAR_SLIDER_FLOAT)
        .CVar("gArcadeKart.PostFx.ShakeResponsePower")
        .Options(FloatSliderOptions()
                     .Min(0.25f)
                     .Max(6.0f)
                     .DefaultValue(3.0f)
                     .Step(0.05f)
                     .Format("%.2f")
                     .Tooltip("Exponential response curve applied only to screen-shake drivers."));
    AddWidget(path, "Speed camera wide FOV: %.0f", WIDGET_CVAR_SLIDER_FLOAT)
        .CVar("gArcadeKart.Camera.SpeedWideFov")
        .Options(FloatSliderOptions()
                     .Min(40.0f)
                     .Max(120.0f)
                     .DefaultValue(100.0f)
                     .Step(1.0f)
                     .Format("%.0f")
                     .Tooltip("Camera FOV at the low end of the shared speed response curve."));
    AddWidget(path, "Speed camera narrow FOV: %.0f", WIDGET_CVAR_SLIDER_FLOAT)
        .CVar("gArcadeKart.Camera.SpeedNarrowFov")
        .Options(FloatSliderOptions()
                     .Min(40.0f)
                     .Max(120.0f)
                     .DefaultValue(60.0f)
                     .Step(1.0f)
                     .Format("%.0f")
                     .Tooltip("Camera FOV at the high end of the shared speed response curve."));
    AddWidget(path, "Speed camera position shift", WIDGET_CVAR_CHECKBOX)
        .CVar("gArcadeKart.Camera.SpeedPositionShift")
        .Options(CheckboxOptions()
                     .DefaultValue(true)
                     .Tooltip("Moves the chase camera down and back using the shared speed response curve."));
    AddWidget(path, "Speed camera near back: %.1f", WIDGET_CVAR_SLIDER_FLOAT)
        .CVar("gArcadeKart.Camera.PositionNearBack")
        .Options(FloatSliderOptions()
                     .Min(0.0f)
                     .Max(120.0f)
                     .DefaultValue(45.0f)
                     .Step(0.5f)
                     .Format("%.1f")
                     .Tooltip("Camera distance behind the kart at zero speed."));
    AddWidget(path, "Speed camera near up: %.1f", WIDGET_CVAR_SLIDER_FLOAT)
        .CVar("gArcadeKart.Camera.PositionNearUp")
        .Options(FloatSliderOptions()
                     .Min(-20.0f)
                     .Max(40.0f)
                     .DefaultValue(12.0f)
                     .Step(0.5f)
                     .Format("%.1f")
                     .Tooltip("Camera height above the kart at zero speed."));
    AddWidget(path, "Speed camera far back: %.1f", WIDGET_CVAR_SLIDER_FLOAT)
        .CVar("gArcadeKart.Camera.PositionFarBack")
        .Options(FloatSliderOptions()
                     .Min(0.0f)
                     .Max(120.0f)
                     .DefaultValue(60.0f)
                     .Step(0.5f)
                     .Format("%.1f")
                     .Tooltip("Camera distance behind the kart at full speed."));
    AddWidget(path, "Speed camera far up: %.1f", WIDGET_CVAR_SLIDER_FLOAT)
        .CVar("gArcadeKart.Camera.PositionFarUp")
        .Options(FloatSliderOptions()
                     .Min(-20.0f)
                     .Max(40.0f)
                     .DefaultValue(3.0f)
                     .Step(0.5f)
                     .Format("%.1f")
                     .Tooltip("Camera height above the kart at full speed."));
    ...
    AddWidget(path, "HUD overscan compensation: %.0f%%", WIDGET_CVAR_SLIDER_FLOAT)
        .CVar("gArcadeKart.PostFx.HudOverscanCompensation")
        .Options(FloatSliderOptions()
                     .Min(0.0f)
                     .Max(1.5f)
                     .DefaultValue(1.0f)
                     .Step(0.05f)
                     .Format("%.2f")
                     .Tooltip("Reserved for HUD layout compensation while post effects render under the HUD."));
    ...
    AddWidget(path, "Post FX warp output scale: %.2f", WIDGET_CVAR_SLIDER_FLOAT)
        .CVar("gArcadeKart.PostFx.WarpOutputScale")
        .Options(FloatSliderOptions()
                      .Min(0.0f)
                      .Max(8.0f)
                      .DefaultValue(4.0f)
                      .Step(0.05f)
                      .Format("%.2f")
                      .Tooltip("Final fisheye/barrel multiplier. 4.0 doubles the previous warp output."));
    ...
    AddWidget(path, "Post FX shake output scale: %.2f", WIDGET_CVAR_SLIDER_FLOAT)
        .CVar("gArcadeKart.PostFx.ShakeOutputScale")
        .Options(FloatSliderOptions()
                     .Min(0.0f)
                     .Max(2.0f)
                     .DefaultValue(0.3f)
                     .Step(0.05f)
                     .Format("%.2f")
                     .Tooltip("Final screen-shake multiplier. 0.3 is 60% of the previous maximum."));
    AddWidget(path, "Post FX shake fast frequency: %.0f Hz", WIDGET_CVAR_SLIDER_FLOAT)
        .CVar("gArcadeKart.PostFx.ShakeFrequencyFastHz")
        .Options(FloatSliderOptions()
                      .Min(0.0f)
                      .Max(120.0f)
                      .DefaultValue(60.0f)
                      .Step(1.0f)
                      .Format("%.0f")
                      .Tooltip("Screen-shake frequency on smooth surfaces."));
    AddWidget(path, "Post FX shake slow frequency: %.0f Hz", WIDGET_CVAR_SLIDER_FLOAT)
        .CVar("gArcadeKart.PostFx.ShakeFrequencySlowHz")
        .Options(FloatSliderOptions()
                      .Min(0.0f)
                      .Max(120.0f)
                      .DefaultValue(8.0f)
                      .Step(1.0f)
                      .Format("%.0f")
                      .Tooltip("Screen-shake frequency on maximum roughness."));
    AddWidget(path, "Post FX shake frequency roughness curve: %.2f", WIDGET_CVAR_SLIDER_FLOAT)
        .CVar("gArcadeKart.PostFx.ShakeFrequencyRoughnessPower")
        .Options(FloatSliderOptions()
                      .Min(0.01f)
                      .Max(8.0f)
                      .DefaultValue(0.5f)
                      .Step(0.05f)
                      .Format("%.2f")
                      .Tooltip("Exponent applied to inverse surface roughness before choosing shake frequency."));
